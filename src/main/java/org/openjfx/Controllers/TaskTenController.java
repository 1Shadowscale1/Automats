...
public class TaskTenController {
    @FXML
    ...
    void initialize() {
        ...
        NFAutomaton suffixAutomaton = (NFAutomaton) automatonList.get(1);
        TableView<String[]> suffixAutomatonTableView = createNFAutomatonJumpTableTableView(suffixAutomaton);
        ...
        Text suffixAutomatonInfo = new Text("Суффиксное замыкание исходного автомата");
        suffixAutomatonInfo.setFill(Color.WHITESMOKE);
        suffixAutomatonInfo.setFont(Font.font("System", 20));

        var returnToStartButton = new Button("Вернуться в начало");
        setupButtonAsReturnToStart(returnToStartButton);

        Button transformNFA2DFAButton = getDFAButton(suffixAutomaton);

        setupMainPaneForAutomatonsTableViewDisplay(mainPane, automatonTableView, suffixAutomatonTableView,
                automatonInfo, suffixAutomatonInfo, returnToStartButton, transformNFA2DFAButton);
    }
